...
#define SIG( x ) if(glw_state.log_fp) fprintf( glw_state.log_fp, x "\n" )
...
static GLboolean APIENTRY logAreTexturesResident( GLsizei n, const GLuint *textures, GLboolean *residences ) {
	...
	return qtrue; //dllAreTexturesResident( n, textures, residences );
}
...
static GLuint APIENTRY logGenLists( GLsizei range ) {
	SIG( "glGenLists" );
	return qtrue; //dllGenLists( range );
}
...
static GLint APIENTRY logRenderMode( GLenum mode ) {
	SIG( "glRenderMode" );
	return qtrue; //dllRenderMode( mode );
}
...
qboolean QGL_DMY_Init( const char *dllname ) {
	...
	// check logging
	QGL_DMY_EnableLogging( r_logFile->integer );
