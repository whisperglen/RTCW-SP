...
#ifdef _DEBUG
#define TRY_DEBUGBREAK() __debugbreak()
#else
#define TRY_DEBUGBREAK()
#endif
...
void qdx_surface_aabb_prune_storage(aabb_group_t* grp)
{
	...
	int original_size = grp->storage.size();
	if (!original_size) return;

	// Create a translation table to track index changes.
	// Initialize with -1 (meaning "this index was deleted/pruned")
	std::vector<int> remap_table(original_size, -1);

	slow = &grp->storage[0];
	slow_idx = 0;
	int steps = original_size;

	// Skip over the valid elements at the start of the array
	for ( ; steps > 0 && slow->usage_count; steps-- )
	{
		remap_table[slow_idx] = slow_idx; // Index didn't change
		slow++;
		slow_idx++;
	}
	...
	if (steps > 0)
	{
		...
		// Shift elements down and record their new locations
		for ( ; steps > 0; steps-- )
		{
			if ( fast->usage_count )
			{
				memcpy( slow, fast, sizeof( aabb_store_t ) );
				// Record that the element at 'fast_idx' is now at 'slow_idx'
				remap_table[fast_idx] = slow_idx;
	...
	if ( newsize < grp->storage.size() )
	{
		...
		// Batch-update the map in ONE single pass
		for (auto it = grp->indexes.begin(); it != grp->indexes.end(); )
		{
			int old_idx = it->second;
			int new_idx = remap_table[old_idx];

			if (new_idx == -1)
			{
				// A surface was mapped to an AABB that got pruned!
				// This means an AABB hit usage_count == 0, but a surface was still pointing to it.
				TRY_DEBUGBREAK();
				//it = grp->indexes.erase(it);
			}
			else
			{
				// Update to the new shifted index
				it->second = new_idx;
				it++;
			}
		}
...
{
	int storage_size = grp->storage.size();

	// 1. Create a temporary array to tally up the REAL usage counts
	std::vector<int> expected_usage(storage_size, 0);

	// 2. Validate mappings and calculate expected counts
	for (auto it = grp->indexes.begin(); it != grp->indexes.end(); it++)
	{
		int index = it->second;

		// CHECK A: Is the index out of bounds of the actual storage vector?
		qassert(index >= 0 && index < storage_size);
		if (index < 0 || index >= storage_size)
		{
			TRY_DEBUGBREAK();
			return index; // Mapping points to an invalid memory location
		}

		// Tally up how many surfaces ACTUALLY point to this index
		expected_usage[index]++;
	}

	// 3. Validate the storage elements against our calculated truth
	for (int i = 0; i < storage_size; i++)
	{
		// CHECK B: Does the stored usage_count perfectly match the mapped count?
		qassert(grp->storage[i].usage_count == expected_usage[i]);
		if (grp->storage[i].usage_count != expected_usage[i])
		{
			TRY_DEBUGBREAK();
			return i; // Usage count drifted (bug in merge/prune logic)
		}

		if (r_showaabbs->integer == 1)
		{
			// CHECK C: Ensure no dead elements exist (assuming this runs after prune)
			qassert(grp->storage[i].usage_count > 0);
			if (grp->storage[i].usage_count == 0)
			{
				TRY_DEBUGBREAK();
				return i; // Ghost AABB detected
			}
		}
	}

	return -1; // State is 100% valid
}
...
int qdx_surface_aabb_generate( const void* surface, aabb_store_t* aabb )
{
	...
	aabb->usage_count = 0;
	aabb->surftype = SRFT_UNK;

	surfaceType_t type = *(surfaceType_t*)surface;

	if ( type == SF_FACE )
	{
		aabb->surftype = SRFT_FACE;
...
#else
		drawVert_t* dv;
		for (i = 0, dv = tris->verts; i < tris->numVerts; i++, dv++)
		{
			AddPointToBounds(dv->xyz, aabb->vmin, aabb->vmax);
		}
#endif
	}
...
int qdx_surface_aabb_get_index( int grpid, const struct msurface_s *msurf, int dbgpoi )
{
	...
	if ( qdx_surface_aabb_generate( surfdata, aabb ) )
	{
		...
		{
			aabb_store_t* it_ptr = &(*it);
			if ( qdx_surface_aabb_intersect( it_ptr, aabb ) )
			{
				index = i;
				qdx_surface_aabb_merge( aabb, it_ptr, it_ptr );
		...
		do {
			...
			{
				aabb_store_t* it_ptr = &(*it);
				//skip over our own index and over empty aabbs
				if ( i != index && it->usage_count && qdx_surface_aabb_intersect(it_ptr, aabb ) )
				{
					//if ( i < index )
					//{
					//	qdx_surface_aabb_merge( aabb, it_ptr, it_ptr );
					...
					//else
					{
						qdx_surface_aabb_merge( aabb, it_ptr, aabb );
						it->usage_count = 0;
...
int qdx_surface_aabb_get_data(int grpid, int index, const aabb_store_t**out)
{
	int ret = 0;
	aabb_group_t* grp = &g_aabb_groups[grpid];
	if (out && index < grp->storage.size())
	{
		*out = &grp->storage[index];
		ret = 1;
	}
	return ret;
}
...
{
	origin[0] = (box->vmin[0] + box->vmax[0]) * 0.5f;
	origin[1] = (box->vmin[1] + box->vmax[1]) * 0.5f;
	origin[2] = (box->vmin[2] + box->vmax[2]) * 0.5f;
}
...
void qdx_surface_aabb_merge (const aabb_store_t* boxa, const aabb_store_t* boxb, aabb_store_t* boxo)
{
	...
	uint16_t count = boxa->usage_count + boxb->usage_count;
	boxo->usage_count = count;
	uint16_t stype = boxa->surftype | boxb->surftype;
	boxo->surftype = stype;
}
...
void qdx_surface_aabb_draw( int grpid )
{
	...
	qdx_vbuffer_steps( STEP_ALLOCATE, &vbuf, VATTID_VERTCOL, 0, MAX_BATCH * 8 * sizeof(vatt_vertcol), 0 );
	qdx_ibuffer_t ibuf = NULL;
	qdx_ibuffer_steps( STEP_ALLOCATE, &ibuf, D3DFMT_INDEX16, 0, MAX_BATCH * 24 * sizeof(uint16_t), 0 );

	// ---------------------------------------------------------
	// 1. PRE-FILL THE INDEX BUFFER
	// ---------------------------------------------------------
	const uint16_t base_lines[] = {
		0, 1,  1, 2,  2, 3,  3, 0,
		4, 5,  5, 6,  6, 7,  7, 4,
		5, 1,  4, 2,  7, 3,  6, 0
	};
	...
	uint16_t* ip;
	qdx_ibuffer_steps( STEP_GET_DATAPTR_CLEAR, &ibuf, D3DFMT_INDEX16, 0, MAX_BATCH * 24 * sizeof(uint16_t), (void**)&ip );

	for (int b = 0; b < MAX_BATCH; b++) {
		for (int l = 0; l < 24; l++) {
			// Offset the indices by 8 for every subsequent box in the batch
			ip[b * 24 + l] = base_lines[l] + (b * 8);
		}
	}
	qdx_ibuffer_steps(STEP_FINALIZE, &ibuf, 0, 0, 0, 0);

	// ---------------------------------------------------------
	// 2. SETUP GLOBAL RENDER STATES
	// ---------------------------------------------------------
	DX9_BEGIN_SCENE();
	GL_State(GLS_DEPTHMASK_TRUE);

	IDirect3DDevice9_SetFVF(qdx.device, VATTID_VERTCOL);
	D3DXMatrixIdentity((D3DXMATRIX*)&qdx.world);

	qdx_matrix_apply();
	qdx_texobj_apply(TEXID_NULL, 0);
	qdx_texobj_apply(TEXID_NULL, 1);

	qdx.device->SetIndices(ibuf); // The index buffer is static for all batches
	...
	int i = 0;
	int boxes_in_batch = 0;
	bool buffer_locked = false;
	vatt_vertcol* v;
	qdx_vbuffer_steps(STEP_GET_DATAPTR_CLEAR, &vbuf, VATTID_VERTCOL, 0, MAX_BATCH * 8 * sizeof(vatt_vertcol), (void**)&v);

	// ---------------------------------------------------------
	// 3. BATCHED VERTEX LOOP
	// ---------------------------------------------------------
	for ( auto it = grp->storage.begin(); it != grp->storage.end(); it++, i++ )
	{
		aabb_store_t* aabb = &(*it);
		if ( aabb->usage_count == 0 && r_showaabbs->integer < 2 )
			continue;

		bool is_colored = (i == g_selected_aabb);

		// Fill 8 vertices for the current box
		v->COLOR = is_colored ? D3DCOLOR_XRGB(0, 0, 0) : white;
		v->XYZ[0] = aabb->vmin[0]; v->XYZ[1] = aabb->vmin[1]; v->XYZ[2] = aabb->vmin[2]; v++;

		v->COLOR = is_colored ? D3DCOLOR_XRGB(0, 0, 255) : white;
		v->XYZ[0] = aabb->vmin[0]; v->XYZ[1] = aabb->vmin[1]; v->XYZ[2] = aabb->vmax[2]; v++;

		v->COLOR = is_colored ? D3DCOLOR_XRGB(255, 0, 255) : white;
		v->XYZ[0] = aabb->vmax[0]; v->XYZ[1] = aabb->vmin[1]; v->XYZ[2] = aabb->vmax[2]; v++;

		v->COLOR = is_colored ? D3DCOLOR_XRGB(255, 0, 0) : white;
		v->XYZ[0] = aabb->vmax[0]; v->XYZ[1] = aabb->vmin[1]; v->XYZ[2] = aabb->vmin[2]; v++;

		v->COLOR = is_colored ? D3DCOLOR_XRGB(128, 128, 128) : white;
		v->XYZ[0] = aabb->vmax[0]; v->XYZ[1] = aabb->vmax[1]; v->XYZ[2] = aabb->vmax[2]; v++;

		v->COLOR = is_colored ? D3DCOLOR_XRGB(0, 255, 255) : white;
		v->XYZ[0] = aabb->vmin[0]; v->XYZ[1] = aabb->vmax[1]; v->XYZ[2] = aabb->vmax[2]; v++;

		v->COLOR = is_colored ? D3DCOLOR_XRGB(0, 255, 0) : white;
		v->XYZ[0] = aabb->vmin[0]; v->XYZ[1] = aabb->vmax[1]; v->XYZ[2] = aabb->vmin[2]; v++;

		v->COLOR = is_colored ? D3DCOLOR_XRGB(255, 255, 0) : white;
		v->XYZ[0] = aabb->vmax[0]; v->XYZ[1] = aabb->vmax[1]; v->XYZ[2] = aabb->vmin[2]; v++;

		boxes_in_batch++;

		// If batch is full, unlock and draw
		if (boxes_in_batch == MAX_BATCH)
		{
			qdx_vbuffer_steps(STEP_FINALIZE, &vbuf, 0, 0, 0, 0);
			qdx.device->SetStreamSource(0, vbuf, 0, sizeof(vatt_vertcol));
			// 8 vertices per box, 12 primitives (lines) per box
			qdx.device->DrawIndexedPrimitive(D3DPT_LINELIST, 0, 0, boxes_in_batch * 8, 0, boxes_in_batch * 12);
