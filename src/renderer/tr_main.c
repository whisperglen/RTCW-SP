...
//	if(curfog->mode != setfog.mode || !setfog.registered) {
	...
	IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGCOLOR, D3DCOLOR_COLORVALUE(curfog->color[0], curfog->color[1], curfog->color[2], 1.0f));
	...
	IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGDENSITY, *(DWORD *)(&curfog->density));
	...
	if (backEnd.refdef.rdflags & RDF_SNOOPERVIEW) {
		...
		IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGSTART, *(DWORD *)(&curfog->end));
	} else {
		//qglFogf(GL_FOG_START, curfog->start);
		IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGSTART, *(DWORD *)(&curfog->start));
	}
...
//		if(setfog.end != r_zfar->value || !setfog.registered) {
		...
		IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGEND, *(DWORD *)(&r_zfar->value));
		...
		if (backEnd.refdef.rdflags & RDF_SNOOPERVIEW) {
			...
			float tmp = curfog->end + 1000;
			IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGEND, *(DWORD *)(&tmp));
		}
		//				qglFogf (GL_FOG_END, curfog->end);
		else {
			//qglFogf(GL_FOG_END, curfog->end);
			IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGEND, *(DWORD *)(&curfog->end));
		}
	...
	qdx.clearColor = D3DCOLOR_COLORVALUE(curfog->color[0], curfog->color[1], curfog->color[2], curfog->color[3]);
...
void R_FogOn(void) {
	...
	IDirect3DDevice9_SetRenderState(qdx.device, D3DRS_FOGENABLE, TRUE);
	//qglEnable( GL_FOG );
	fogIsOn = qtrue;
}
// done.
...
void R_RotateForEntity(const trRefEntity_t *ent, const viewParms_t *viewParms,
	...
	memcpy(&qdx.world.m[0][0], glMatrix, sizeof(qdx.world.m));
...
void R_DebugPolygon(int color, int numPoints, float *points) {
	...
	qdx_vbuffer_steps(STEP_ALLOCATE|STEP_GET_DATAPTR_CLEAR, &qbuf, VATTID_VERTCOL, 0, sizeof(vatt_vertcol_t) * numPoints, &data);
	...
	qdx_vbuffer_steps(STEP_FINALIZE, &qbuf, VATTID_VERTCOL, 0, sizeof(vatt_vertcol_t) * numPoints, NULL);
	...
	//qglBegin(GL_POLYGON);
	qdx_vbuffer_steps(STEP_GET_DATAPTR_CLEAR, &qbuf, VATTID_VERTCOL, 0, sizeof(vatt_vertcol_t) * numPoints, &data);
	for (i = 0; i < numPoints; i++) {
		//qglVertex3fv(points + i * 3);
		memcpy(data->XYZ, points + i * 3, sizeof(data->XYZ));
		data->COLOR = dxcol;
